...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams RIGHT JOIN games ON teams.team_id IN(games.winner_id, games.opponent_id) WHERE round = 'Eighth-Final' AND year = 2014")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams RIGHT JOIN games ON teams.team_id IN(games.winner_id, games.opponent_id) WHERE name LIKE 'Co%'")"
